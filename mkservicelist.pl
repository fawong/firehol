...
if (@ARGV != 3) {
  print STDERR "Usage: mkservicedoc.pl output.xml firehol-script services-data.txt\n";
  exit 1;
}
...
foreach my $s (sort(keys(%services))) {
  unless ($db{$s} or $dbalias{$s}) {
    print STDERR "Service $s in firehol but not in services-data.txt\n";
    $err = 1;
  }
  $servs{$s} = 1;
}
...
foreach my $d (sort(keys(%db))) {
  unless ($services{$d}) {
    print STDERR "Service $d services-data.txt but not in firehol\n";
    $err = 1;
  }
  $servs{$d} = 1;
}
